...
static void video_write(void){
  ...
  char dirName[32];
  snprintf(dirName, 32, "output_frames");
  mkdir(dirName, 0777);

  char fileName[32];
  snprintf(fileName, 32, "output_frames/frame_%05d.ppm", sFrameCount++);
  FILE *frameFile = fopen(fileName, "w");

  fprintf(frameFile, "P6\n");
  fprintf(frameFile, "%d %d\n255\n", ti.frame_width, ti.frame_height);
  
  // ycbcr is 3 chroma planes; YCbCr.
  // need to convert that to RGB.

  int yPos, x;

  // assuming planes are the same height and width, hopefully that's safe.
  int width = ycbcr[0].width;
  int height = ycbcr[0].height;
  
  printf("Frame %d: fw%d fh%d pw%d ph%d px%d py%d pixel format ", sFrameCount,
    ti.frame_width,
    ti.frame_height,
    ti.pic_width,
    ti.pic_height,
  ...
  switch (ti.pixel_fmt) {
    case TH_PF_420:printf("TH_PF_420\n");break;
    case TH_PF_422:printf("TH_PF_422\n");break;
    case TH_PF_444:printf("TH_PF_444\n");break;
    default:printf("unknown\n");
  }
  printf("  color planes are: %dx%d %d  %dx%d %d  %dx%d %d\n",
    ycbcr[0].width, ycbcr[0].height, ycbcr[0].stride,
    ...
    for (x=0;x<width;x++) {
      ...
      switch (ti.pixel_fmt) {
        case TH_PF_420:
          ...
          y =  *(ycbcr[0].data + ycbcr[0].stride * yPos + x);
          cb = *(ycbcr[1].data + ycbcr[1].stride * yPos/2 + x/2);
          cr = *(ycbcr[2].data + ycbcr[2].stride * yPos/2 + x/2);
        ...
        case TH_PF_444:
          y =  *(ycbcr[0].data + ycbcr[0].stride * yPos + x);
          cb = *(ycbcr[1].data + ycbcr[1].stride * yPos + x);
          cr = *(ycbcr[2].data + ycbcr[2].stride * yPos + x);
          cr -= 128;
          cb -= 128;
          r = y + cr + (cr>>2) + (cr>>3) + (cr>>5);
          g = y - ((cb >> 2) + (cb>>4) + (cb>>5)) - ((cr >> 1) + (cr>>3) + (cr >> 4) + (cr >> 5));
        ...
        case TH_PF_422:
        case TH_PF_NFORMATS:
          fprintf(stderr, "Sorry, unsupported pixel format.\n");
          exit(1);
      
      }
      unsigned char o;
      o = r;
      fwrite(&o, 1, 1, frameFile);
      o = g;
      fwrite(&o, 1, 1, frameFile);
      o = b;
      fwrite(&o, 1, 1, frameFile);

      //fprintf(frameFile, "%d %d %d ", r, g, b);
    }
    //fprintf(frameFile, "\n");
  }
  fclose(frameFile);
  ...
  if(outfile){
    ...
    for(pli=0;pli<3;pli++){
      for(i=0;i<ycbcr[pli].height;i++){
        fwrite( 
          ycbcr[pli].data + ycbcr[pli].stride * i, 
          1,
          ycbcr[pli].width, 
          outfile);
      }
